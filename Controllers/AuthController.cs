...
namespace FoodTruckRodeo.API.Controllers
{
  ...
  public class AuthController : ControllerBase
  {
    ...
    IAuthRepository repo, IConfiguration config, IMapper mapper
    )
    {
      _mapper = mapper;
      _config = config;
  ...
  public async Task<IActionResult> Register(UserForRegisterDTO userForRegisterDTO, int foodTruckId)
  {
    ...
    // return StatusCode(201); // temp fix 
    
    var userToReturn = _mapper.Map<UserForListDTO>(createdUser);
    
    // need to return CreatedAtRoute - location header with the requests as well as the new resource created
